  ...
  <Dialog v-model="dialog" :width="640">
    <Card>
      ...
      <v-card-text>
        ...
        <br />
        <Label header>Name : "{{ animal.name }}"" </Label>
      </v-card-text>
      <v-card-actions>
...
const emit = defineEmits(["update:modelValue", "update:animal", "removed"]);

const isLoading = ref(false);
const dialog = computed(useModel(propRef, emit, "modelValue"));
const animal = computed(useModel(propRef, emit, "animal"));
