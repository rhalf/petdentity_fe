  ...
  <Dialog v-model="dialog" :width="640">
    <Card>
      <v-card-title class="bg-primary pa-4">
        <Label header class="text-black"> Remove Coat</Label>
      </v-card-title>
      <v-card-text>
        <Label text> Are you sure you want to remove this item?</Label>
        <br />
        <Label header>Name : "{{ coat.name }}" </Label>
      </v-card-text>
      <v-card-actions>
...
import { remove } from "@/api/coat";

import { useModel } from "@/utils/vue";

import { ref, computed, toRefs } from "vue";
const props = defineProps({ modelValue: Boolean, coat: Object });
const propRef = toRefs(props);
const emit = defineEmits(["update:modelValue", "update:coat", "removed"]);

const isLoading = ref(false);
const dialog = computed(useModel(propRef, emit, "modelValue"));
const coat = computed(useModel(propRef, emit, "coat"));

const submitHandler = async () => {
  try {
    isLoading.value = true;
    await remove(coat.value);
